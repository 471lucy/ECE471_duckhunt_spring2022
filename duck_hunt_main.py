...
def main(args):
    ...
    #folder_ index - for data collection remove later
    folder_in = 1
    ...
    while True:
        ...
        if args.move_type == 'manual':
            ...
            if future is None:
                ...
                future = executor.submit(GetLocation, args.move_type, env, current_frame, folder_in) #added folder_in for data collection remove later
       ...
       # print(result)
        for res in result:
            ...
            #maybe this will work idk trying to go to multiple coorditates
            ...
            if type(coordinate) == list:
                for i in range(len(coordinate)):
                    #print('yes', coordinate[i])
                    current_frame, level_done, game_done, info = env.step(coordinate[i], move_type)
